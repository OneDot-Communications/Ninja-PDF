...
export default function SuperAdminAdminsPage() {
    ...
    const loadAdmins = async () => {
        try {
            ...
            const filtered = users.filter((u: any) => u.role === 'ADMIN' || u.role === 'SUPER_ADMIN');
            setAdmins(filtered);
        } catch (error) {
                                ...
                                <TableCell>
                                    <div className="flex items-center gap-3">
                                        <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center font-semibold text-slate-600">
                                            {(admin.first_name?.[0] || admin.email?.[0] || 'U').toUpperCase()}
                                        </div>
                                        <div>
                                            <p className="font-medium text-slate-900">{admin.first_name ? `${admin.first_name} ${admin.last_name || ''}` : admin.email}</p>
